...
import { searchTeams, colorFromId, placeholderLogo } from '../api/f1Api';
...
export default function TeamSearch() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (!query) { setTeams([]); setTotal(0); return; }
      const { items, total } = await searchTeams(query, limit, offset);
      if (cancelled) return;
      setTotal(total);
      const mapped = items.map(t => {
        ...
        return {
          ...
          name: t.teamName,
          color,
          logo: placeholderLogo(t.teamName.split(' ').slice(0,2).map(w=>w[0]).join(''), '#444444')
        };
      });
      const exact = mapped.find(m => m.name.toLowerCase() === query.toLowerCase());
      if (exact) setTeams([exact, ...mapped.filter(m=>m.id!==exact.id)]); else setTeams(mapped);
    })();
  ...
  return (
    <section className="bg-gray-100/50 py-10 sm:py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <form onSubmit={e => { e.preventDefault(); navigate(`/teams/search/${encodeURIComponent(input.trim())}`); }} className="mb-6 flex flex-wrap gap-2 items-center">
          <input
            value={input}
            onChange={e=>{ setInput(e.target.value); clearTimeout(typingTimer.current); typingTimer.current=setTimeout(()=>navigate(`/teams/search/${encodeURIComponent(e.target.value.trim())}`),400); }}
        ...
        <div className="mb-4 flex justify-between items-center">
          ...
          <div className="text-sm text-gray-600">Showing {teams.length?offset+1:0}-{offset+teams.length} of {total}</div>
